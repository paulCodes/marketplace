...
You are alpha testing Tab for Projects. While using any `mcp__tab-for-projects__*` tool or any `tab-*` skill, actively observe and log feedback to `~/workspaces/notes/tab-feedback-report.md`.
...
Agent(
  ...
  prompt: "Append this entry to ~/workspaces/notes/tab-feedback-report.md: ...",
  run_in_background: true
)
```

Append to `~/workspaces/notes/tab-feedback-report.md`. Use this format for new entries:
...
The user will periodically run `/tab-feedback` to compile the report, or just hand `~/workspaces/notes/tab-feedback-report.md` directly to the Tab creator. Keep it clean enough that someone who didn't watch the session can understand each entry.
